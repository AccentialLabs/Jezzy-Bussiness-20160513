...
class UtilsController extends AppController {
    ...
    public function sendMobileNotification($userId, $message) {
        ...
        $pusher = new Pusher(
                '9f4cbd00132ee1e897fd', '3201f7c0f934526ce629', '189862'
        );
    ...
    public function NASPMoip() {
        ...
        if ($this->request->is('post')) {
            ...
            $selectCheckout = "select * from checkouts inner join offers on offers.id = checkouts.offer_id inner join users on users.id = checkouts.user_id inner join companies on companies.id = checkouts.company_id where checkouts.id = {$transactionId};";
