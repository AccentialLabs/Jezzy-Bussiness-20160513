                    ...
                    if (is_array($allClients)) {
                        foreach ($allClients as $client) {
                            if (!empty(trim($client['User']['lastCheckout']))) {
                                $lastCheckoutDate = $client['User']['lastCheckout'] = date('d/m/Y', strtotime($client['User']['lastCheckout']));
                            }else{
                                 $lastCheckoutDate =  'Nenhuma compra realizada';
                            }
                            if (!empty($client['User']['lastSchedule'])) {
                               $lastScheduleDate = date('d/m/Y', strtotime($client['User']['lastSchedule'][0]['schedules']['date']));
                            }else{
